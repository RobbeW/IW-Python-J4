...
def twin_prime(getal):
    # Finds the last twin prime
    p1 = getal - 1
    while not (is_priem(p1) and is_priem(p1 - 2)):
        p1 -= 1
    return p1
...
for i in range(len(cases)):
    ...
    # generate test expression
    a = twin_prime(test[0])
    expression_name = f"is_priem({a})"
    result = module.is_priem(a)

    # setup for return expressions
    testcase = { "expression": expression_name, "return": result }
    yamldata[0]['contexts'][i]["testcases"].append( testcase)
    
    expression_name = f"is_priem({a - 1})"
    result = module.is_priem(a - 1)

    # setup for return expressions
    testcase = { "expression": expression_name, "return": result }
    yamldata[0]['contexts'][i]["testcases"].append( testcase)
    
    expression_name = f"is_priem({a - 2})"
    result = module.is_priem(a - 2)
